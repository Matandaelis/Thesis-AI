...
const faqs = [
  {
    ...
    questions: [
      ...
      { q: "Is the AI content plagiarism-free?", a: "Our AI generates original content based on your prompts. However, we always recommend running the built-in Integrity Audit before final submission." },
    ]
  },
  {
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto animate-fade-in pb-20">
      <div className="text-center mb-10">
        <h1 className="text-3xl font-serif font-bold text-zinc-900 mb-4">How can we help you?</h1>
        <div className="max-w-2xl mx-auto relative">
          <input 
            type="text" 
            className="w-full pl-12 pr-4 py-3 rounded-xl border border-zinc-200 shadow-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all text-zinc-900"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-2 space-y-6">
          {filteredFaqs.length === 0 ? (
             <div className="text-center py-8 text-zinc-500">No results found. Try a different keyword.</div>
          ) : (
             filteredFaqs.map((cat, catIdx) => (
                <div key={catIdx} className="bg-white rounded-xl shadow-sm border border-zinc-200 overflow-hidden">
                  <div className="bg-zinc-50 px-6 py-3 border-b border-zinc-100 flex items-center gap-2">
                    <Book size={16} className="text-indigo-600" />
                    <h3 className="font-bold text-zinc-700 text-sm uppercase tracking-wide">{cat.category}</h3>
                  </div>
                    ...
                    {cat.questions.map((item, qIdx) => {
                      ...
                      return (
                        <div key={qIdx} className="group">
                          ...
                          >
                            <span className={`font-medium ${isOpen ? 'text-indigo-700' : 'text-zinc-800'}`}>{item.q}</span>
                            {isOpen ? <ChevronUp size={18} className="text-indigo-500" /> : <ChevronDown size={18} className="text-zinc-400" />}
                          </button>
                          {isOpen && (
                            <div className="px-6 pb-4 text-sm text-zinc-600 leading-relaxed animate-fade-in-down">
                              {item.a}
                            </div>
                          )}
                        </div>
        ...
        <div className="space-y-6">
           <div className="bg-zinc-900 text-white rounded-xl p-6 shadow-lg">
              <h3 className="font-bold text-lg mb-2 flex items-center gap-2">
                <MessageCircle size={20} className="text-indigo-400" /> Still stuck?
              </h3>
              <p className="text-zinc-300 text-sm mb-6">Our academic support team is available to assist you.</p>
              
              <button className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-2 rounded-lg font-bold transition-colors mb-3 flex items-center justify-center gap-2">
           ...
           <div className="bg-white rounded-xl border border-zinc-200 p-6 shadow-sm">
              ...
              <p className="text-xs text-zinc-500 mb-4">Suggest a new tool or university template.</p>
              <textarea 
                className="w-full border border-zinc-200 rounded-lg p-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none resize-none h-24 mb-3"
                placeholder="I wish ScholarSync could..."
              ></textarea>
