      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden order-2 md:order-1">
         ...
         <div className="h-14 md:h-16 bg-white border-b border-slate-200 flex items-center justify-between px-4 md:px-6 shrink-0">
            <div className="flex items-center space-x-2 md:space-x-4">
               <div className="flex items-center bg-slate-100 rounded-lg p-1 border border-slate-200 mr-2 md:mr-4">
                   <button 
                     onClick={() => setViewMode('calendar')}
                     className={`p-1.5 rounded transition-colors ${viewMode === 'calendar' ? 'bg-white shadow text-teal-700' : 'text-slate-500 hover:text-slate-700'}`}
                     title="Month View"
                   >
                   ...
                   <button 
                     ...
                     className={`p-1.5 rounded transition-colors ${viewMode === 'roadmap' ? 'bg-white shadow text-teal-700' : 'text-slate-500 hover:text-slate-700'}`}
                        ...
                        {roadmapStages.map((stage: any) => (
                            <div key={stage.id} className="relative pl-6">
                                <div className={`absolute -left-[25px] top-0 w-4 h-4 rounded-full border-2 ${
                                    ...
                                    stage.status === 'active' ? 'bg-white border-teal-500 animate-pulse' :
                                    'bg-white border-slate-300'
                                }`}></div>
                                
                                <div className={`p-4 rounded-xl border ${
                                    stage.status === 'active' ? 'bg-white border-teal-200 shadow-md' : 
                                    stage.status === 'completed' ? 'bg-slate-50 border-slate-200 opacity-70' :
                                    'bg-white border-slate-100'
                                }`}>
                                    <div className="flex justify-between items-center mb-2">
                                        <h4 className={`font-bold ${stage.status === 'active' ? 'text-teal-700' : 'text-slate-700'}`}>{stage.name}</h4>
                                    ...
                                    <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden">
                                        <div 
                                            className={`h-full rounded-full ${
                                                ...
                                                stage.status === 'active' ? 'bg-teal-500 w-1/2' :
                                                'w-0'
                                            }`}
                                        ></div>
                                    </div>
