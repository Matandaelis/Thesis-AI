      ...
      <div className="w-80 bg-white border-r border-zinc-200 flex flex-col shrink-0">
        <div className="p-4 border-b border-zinc-200 bg-zinc-50/50">
           <h2 className="font-bold text-zinc-800 flex items-center gap-2">
             <BrainCircuit className="text-indigo-600" size={20} /> Lit. Synthesis
           </h2>
           <p className="text-xs text-zinc-500 mt-1">Select papers to analyze collectively.</p>
        </div>

        <div className="flex-1 overflow-y-auto p-2 space-y-1">
           {items.length === 0 ? (
               <div className="p-4 text-center text-zinc-400 text-sm italic">Library is empty.</div>
           ) : items.map(item => (
               <div 
                 key={item.id} 
                 onClick={() => toggleSelection(item.id)}
                 className={`p-3 rounded-lg border cursor-pointer transition-all flex items-start gap-3 group ${selectedIds.has(item.id) ? 'bg-indigo-50 border-indigo-200 shadow-sm' : 'bg-white border-zinc-100 hover:border-zinc-300'}`}
               >
                  <div className={`mt-0.5 ${selectedIds.has(item.id) ? 'text-indigo-600' : 'text-zinc-300 group-hover:text-zinc-400'}`}>
                      {selectedIds.has(item.id) ? <CheckSquare size={16} /> : <Square size={16} />}
                  </div>
                  <div className="flex-1 min-w-0">
                      <h4 className={`text-sm font-bold truncate ${selectedIds.has(item.id) ? 'text-indigo-900' : 'text-zinc-700'}`}>{item.title}</h4>
                      <p className="text-xs text-zinc-500 truncate">{item.author} • {item.year}</p>
                  </div>
               </div>
           ))}
        </div>
        ...
        <div className="p-4 border-t border-zinc-200 bg-white space-y-2">
            ...
            <button 
                ...
                disabled={selectedIds.size < 2 || isProcessing}
                className="w-full py-2 bg-indigo-600 text-white font-bold rounded-lg hover:bg-indigo-700 flex items-center justify-center gap-2 text-sm transition-colors disabled:opacity-50"
            >
      ...
      <div className="flex-1 flex flex-col h-full bg-zinc-50 overflow-hidden">
          ...
          <div className="h-14 bg-white border-b border-zinc-200 flex items-center justify-between px-6 shrink-0">
              <div className="flex items-center gap-4">
                  <span className={`text-sm font-bold px-3 py-1 rounded-full transition-colors ${mode === 'analysis' ? 'bg-indigo-100 text-indigo-700' : 'text-zinc-500 hover:bg-zinc-100'}`}>Analysis</span>
                  <ArrowRight size={14} className="text-zinc-300" />
                  <span className={`text-sm font-bold px-3 py-1 rounded-full transition-colors ${mode === 'draft' ? 'bg-indigo-100 text-indigo-700' : 'text-zinc-500 hover:bg-zinc-100'}`}>Draft Builder</span>
              </div>
              {output && (
                  <button onClick={() => { navigator.clipboard.writeText(output); alert("Copied!"); }} className="text-zinc-400 hover:text-zinc-600 flex items-center gap-1 text-xs font-bold">
                      <Copy size={14} /> Copy
                  </button>
              )}
          </div>
          ...
          <div className="flex-1 p-6 md:p-8 overflow-y-auto">
              {!output && !isProcessing ? (
                  <div className="flex flex-col items-center justify-center h-full text-zinc-400">
                      ...
                      <h3 className="text-lg font-bold text-zinc-600 mb-2">Ready to Synthesize</h3>
                  ...
                  <div className="flex h-full gap-6">
                      <div className="flex-1 bg-white rounded-xl shadow-sm border border-zinc-200 flex flex-col overflow-hidden">
                          <div className="p-3 bg-zinc-50 border-b border-zinc-100 flex justify-between items-center">
                              <span className="text-xs font-bold text-zinc-500 uppercase tracking-wider">AI Output</span>
                              <span className="text-[10px] bg-zinc-200 px-2 py-0.5 rounded text-zinc-600 font-mono">Markdown</span>
                          </div>
                          {mode === 'draft' ? (
                              <textarea 
                                  className="flex-1 w-full p-6 resize-none focus:outline-none text-zinc-800 leading-relaxed font-serif"
                                  value={output}
                                  onChange={(e) => setOutput(e.target.value)}
                              />
                          ) : (
                              <div className="flex-1 w-full p-6 overflow-y-auto prose prose-sm max-w-none prose-headings:font-bold prose-h2:text-indigo-800 prose-li:text-zinc-700">
