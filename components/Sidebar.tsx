...
import { 
  ...
  Layers, PenTool, Database, Briefcase
} from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ currentView, onChangeView }) => {
  ...
  return (
    <div className="w-64 bg-slate-900 text-slate-300 flex flex-col h-full shadow-xl border-r border-slate-800">
            ...
            <div className="space-y-1">
              {section.items.map((item) => {
                ...
                return (
                  <div key={item.label}>
                    <button
                      onClick={() => hasSubItems ? toggleGroup(item.label) : item.id && onChangeView(item.id)}
                       ...
                       <div className="mt-1 ml-4 pl-4 border-l border-slate-700 space-y-1 animate-fade-in-down">
                          {item.subItems?.map(sub => {
                             ...
                             return (
                                <button
                                  key={sub.label}
                                  onClick={() => sub.id && onChangeView(sub.id)}
